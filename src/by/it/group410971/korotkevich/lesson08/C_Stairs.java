...
public class C_Stairs {
    ...
    int getMaxSum(InputStream stream ) {
        ...
        int[] dp = new int[n + 1];
        dp[0] = 0;          // Начальная точка (перед первой ступенькой)
        dp[1] = stairs[0];  // Сумма после первой ступеньки

        for (int i = 2; i <= n; i++) {
            // Выбираем лучший вариант: шаг с предыдущей или с предпредыдущей ступеньки
